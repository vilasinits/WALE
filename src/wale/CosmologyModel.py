...
class Cosmology_function:
    ...
    def _E(self, z):
        ...
        return np.sqrt(
            self.Om*(1+z)**3
            + self.Ok*(1+z)**2
            + self.Ode*(1+z)**(3*(1+self.w+self.wa)) * np.exp(-3*self.wa*z/(1+z))
        )
    ...
    def get_z_from_chi(self, chi_target, z_max=10.0, tol=1e-8):
        ...
        def find_root(chi_t):
            ...
            if chi_t < 0 or chi_t > chi_max:
                raise ValueError(f"chi_target={chi_t:.3f} outside [0, {chi_max:.3f}] Mpc (z_max={z_max})")
    ...
    def get_lensing_weight_array(self, chis, chi_source):
        ...
        for i in range(len(chis)):
            z = z_values[i]
            lensing_weight[i] = 1.5 * self.Om * (self.speed_light**-2.0) * ((self.H0) ** 2.0) * chis[i] * (1 - (chis[i] / chi_source)) * (1 + z)
