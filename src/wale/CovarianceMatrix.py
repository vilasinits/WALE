...
def get_covariance(cosmo,  z, variability, numberofrealisations):
    ...
    Nmodes = vol/3/(2*np.pi**2)*( (cosmo.k+cosmo.dk/2)**3-(cosmo.k-cosmo.dk/2)**3) #Number of k-modes in shells
    ...
    if variability:
        ...
        prof = ccl.halos.HaloProfileNFW(mass_def=hmd_200m, concentration=cM, fourier_analytic=True)
        ...
        Tk = halomod_Tk3D_cNG(
            ...
            a_arr  = np.atleast_1d(scale_factor),         # only one scale factor → 2D interpolation
