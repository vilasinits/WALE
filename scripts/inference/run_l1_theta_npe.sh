#!/usr/bin/env bash
# Single-θ L1 NPE sweep with chosen κ-cuts.
# Args: $1=theta (e.g. 40.0); cuts read from env CUTS (whitespace-separated),
# or from the cut-design CSV when no override is given.
set -e
...
SIM_REALS="data/l1/simulations/sim_doth_l1_bin4_realizations_fidcov_theta${THETA}_ratio2.0.npz"
THEORY_REALS="data/l1/theory/theory_doth_l1_bin4_realizations_fidcov_theta${THETA}_ratio2.0.npz"
SIM_FID="data/l1/simulations/sim_doth_l1_bin4_theta${THETA}_ratio2.0_fiducial.npz"
...
for CUT in "${CUTS[@]}"; do
    ...
    echo "[θ=${THETA} ${TAG}] theory-trained"
    conda run -n jaxili python scripts/inference/run_npe_inference.py \
        --data-key l1_norms --axis-key kappa_bins \
...
echo "[done] θ=${THETA} sweep complete."
