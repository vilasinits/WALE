...
def parse_filename(name: str) -> dict | None:
    ...
    return dict(training=training, theta=theta, cut_tag=tags, name=name)
...
def main() -> None:
    ...
    # Find matching pairs of (sim, theory*) samples per (theta, cut_tag).
    by_cut: dict[tuple[float, str], dict[str, Path]] = {}
    for f in sorted(args.samples_dir.glob("samples_l1_*_npe.npy")):
        ...
        if info is None or info["training"] not in ("sim", "theory", "theory_norecal"):
            continue
        by_cut.setdefault((info["theta"], info["cut_tag"]), {})[info["training"]] = f

    if not by_cut:
        print(f"No L1 sample files found in {args.samples_dir}")
        return

    print(f"Found {len(by_cut)} (theta, cut) cells in {args.samples_dir}.")
    summary_rows = []
    for (theta, cut_tag), files in sorted(by_cut.items()):
        if "sim" not in files:
            print(f"  [skip] theta={theta} cut='{cut_tag}' missing sim.")
            continue
        s_sim = load_samples(files["sim"])
        # Compare sim vs every theory-* training we found in this cell
        theory_variants = [k for k in files if k.startswith("theory")]
        if not theory_variants:
            print(f"  [skip] theta={theta} cut='{cut_tag}' missing theory.")
            continue
        for tname in theory_variants:
            ...
            print(f"  theta={theta} cut={nice_tag}: sim N={s_sim.shape[0]}, "
            ...
            g.fig.suptitle(f"L1 NPE contours: sim vs {tname}, {theta_tag}, cut={nice_tag}",
                           fontsize=12, y=1.01)
            outdir = args.outdir if args.outdir is not None else (
                REPO_ROOT / "outputs" / "plots" / "l1" / theta_tag / "overlays"
            )
            outdir.mkdir(parents=True, exist_ok=True)
            out_pdf = outdir / f"l1_contours_{theta_tag}_cut_{nice_tag}.pdf"
            ...
            summary_rows.append(dict(theta=theta, cut_tag=nice_tag,
                                     shifts=shift,
                                     max_abs=float(np.max(np.abs(shift)))))

    if summary_rows:
        print("\nPer-(theta, cut) posterior shift (theory − sim) / sigma_sim:")
        print(f"  {'theta':>6} {'cut':>30} {'Om':>7} {'sigma8':>8} {'w0':>7} {'H0':>7} {'ns':>7} {'Ob':>7}    max")
        for r in summary_rows:
            shifts = "  ".join(f"{x:+.2f}" for x in r["shifts"])
            print(f"  {r['theta']:>6.1f} {r['cut_tag']:>30}  {shifts}   {r['max_abs']:+.2f}")
